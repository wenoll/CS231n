...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # 无循环：矩阵运算 + 广播
        # L2距离公式展开：||x-y||² = x² + y² - 2xy
        #########################################################################
        # 计算 X² 求和 (num_test,)
        test_sq = np.sum(X**2, axis=1, keepdims=True)
        # 计算 train² 求和 (num_train,)
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            #########################################################################
            #########################################################################
            # 第二步：投票，取出现次数最多的标签；平局选数值小的标签
            #########################################################################
            # 统计每个标签出现的次数
            unique_labels, counts = np.unique(closest_y, return_counts=True)
            # 找到出现次数最多的标签
            most_common_idx = np.argmax(counts)
            y_pred[i] = unique_labels[most_common_idx]
        return y_pred
